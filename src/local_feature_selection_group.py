...
def load_and_manipulate_data(file_path, selected_features_v1, threshold=0.00000, cluster_number=-1):
    ...
    selected_features_ = selected_features_[
        selected_features_["Cluster_Probabilities"] >= threshold
    ]

    # Add the 'Target' column
    selected_features_["Target"] = np.where(
        selected_features_["Cluster_Labels"] == cluster_number, 1, 0
    )
    ...
    def calculate_best_alpha(self):
        ...
        for alpha in alphas:
            ...
            cv_scores = cross_val_score(
                lasso_classifier, self.X_train, self.y_train, cv=skf, scoring="f1_micro"
            )
...
for year in unique_years:
    if year == 2021:  # Condition to limit the loop to the year 2021
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X_train_scaled, y_target, test_size=0.2, random_state=0
        )

        # Fit a Lasso model
        # Create an instance of the LassoClassifierAnalyzer
        lasso_analyzer = LassoClassifierAnalyzer(
        ...
        lasso_coef, lasso_intercept, lasso_train_score, lasso_test_score = (
            lasso_analyzer.analyze_lasso(alpha=best_alpha)
        )
        ...
        for _ in range(num_iterations):
            ...
            feature_importance_analyzer = FeatureImportanceAnalyzer(
                feature_names=feature_names, lasso_coef=lasso_coef
            )
            ...
            for index, row in top_features.iterrows():
                ...
                if feature_name in feature_count_df["Feature_Name"].values:
                    ...
                    feature_count_df.loc[
                        feature_count_df["Feature_Name"] == feature_name, "Count"
                    ] += 1
                else:
                    # If it doesn't exist, add it to the DataFrame with a count of one
                    feature_count_df = feature_count_df.append(
                        {"Feature_Name": feature_name, "Count": 1}, ignore_index=True
                    )
