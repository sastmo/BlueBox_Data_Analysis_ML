...
from treeinterpreter import utils
...
def load_and_manipulate_data(file_path, selected_features_v1):
    ...
    selected_features_ = clustered_data.loc[
        :, clustered_data.columns.isin(selected_features_v1_extend)
    ]
...
def analyze_feature_contributions_treeinterpreter(
    model, X_train_scaled2, X_feature, program_codes_for_compression
):
    """
    ...
    for i in range(len(program_codes_for_compression)):
        ...
        for c, feature in sorted(
            zip(contributions[i], X_feature.columns), key=lambda x: -abs(x[0])
        ):
            print(feature, round(c, 2))
        print("-" * 20)

    # Step 3: Aggregated Contribution Analysis between two rows
    prediction1, bias1, contributions1 = ti.predict(
        model, np.array([X_feature.iloc[0]]), joint_contribution=True
    )
    prediction2, bias2, contributions2 = ti.predict(
        model, np.array([X_feature.iloc[1]]), joint_contribution=True
    )
    ...
    for k in set(aggregated_contributions1.keys()).union(set(aggregated_contributions2.keys())):
        res.append(
            (
                [X_feature.columns[k] for k in k],
...
def analyze_feature_contributions_LIME(model, X_train_, X_feature, program_codes_for_compression):
    ...
    for i_, program_code in enumerate(program_codes_for_compression):
        ...
        exp_program_codes_for_compression[i_] = explainer.explain_instance(
            X_train.values[i_], model.predict, num_features=5
        )

        # Get the explanation as a list of tuples
        explanation_text_program_codes_for_compression[i_] = exp_program_codes_for_compression[
            i_
        ].as_list()

        # Get the explanation as a pyplot figure
        explanation_plot_program_codes_for_compression[i_] = exp_program_codes_for_compression[
            i_
        ].as_pyplot_figure()
...
selected_features = load_and_manipulate_data(
    DATA_DIR / "result_classification.csv", selected_features_v1
)
...
for year in unique_years:
    if year == 2021:
        ...
        y_target_global = selected_features_df[
            "TOTAL Reported and/or Calculated Marketed Tonnes"
        ].copy()
        X_feature_global = selected_features_df.drop(
            ["Year", "Program Code", "TOTAL Reported and/or Calculated Marketed Tonnes"], axis=1
        ).copy()

        X_feature_global_temp = selected_features_df.drop(
            ["Year", "TOTAL Reported and/or Calculated Marketed Tonnes"], axis=1
        ).copy()

        X_feature_global = pd.get_dummies(
            X_feature_global, columns=["Municipal Group"], prefix="Municipal Group"
        )
        X_feature_global_temp = pd.get_dummies(
            X_feature_global_temp, columns=["Municipal Group"], prefix="Municipal Group"
        )
        ...
        X_feature = X_feature_global_temp[
            X_feature_global_temp["Program Code"].isin(program_codes_for_compression)
        ]
        X_feature = X_feature.drop(["Program Code"], axis=1).copy()

        y_target = selected_features_df[
            selected_features_df["Program Code"].isin(program_codes_for_compression)
        ...
        analyze_feature_contributions_treeinterpreter(
            model, X_train_scaled2, X_feature, program_codes_for_compression
        )

        # Analyze local feature Contribution using LIME
        analyze_feature_contributions_LIME(
            model, X_train_scaled2, X_feature, program_codes_for_compression
        )
