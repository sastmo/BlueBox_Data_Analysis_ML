...
def create_scatter_plot(
    ax, x_values, y_values, x_label, y_label, program_codes, group_municipality
):
    """
    ...
    scatter = sns.scatterplot(
        x=x_values, y=y_values, hue=group_municipality, palette=colors, ax=ax, alpha=0.7
    )
    ...
    ax.plot(
        ...
        y_fit,
        color="orange",
        label=f"L_Reg (R²={r_value ** 2:.2f}, p={p_value:.2f}, cor={slope:.2f})",
    )
    ...
    hover_text = [
        f"Program Code: {code}, group_municipality: {group}"
        for code, group in zip(program_codes, group_municipality)
    ]
...
def visualize_households_vs_operation_cost(market_agg, unique_years):
    ...
    for year in unique_years:
        ...
        normalized_operation_cost = (
            year_data["operation cost"] - year_data["operation cost"].min()
        ) / (year_data["operation cost"].max() - year_data["operation cost"].min())

        # Normalize 'Total Households Serviced' for the current year
        normalized_total_households = (
            year_data["Total Households Serviced"] - year_data["Total Households Serviced"].min()
        ) / (
            year_data["Total Households Serviced"].max()
            - year_data["Total Households Serviced"].min()
        )
        ...
        create_scatter_plot(
            ...
            normalized_operation_cost,
            normalized_total_households,
            "Normalized Operation Cost",
            "Normalized Total Households Serviced",
            year_data["Program Code"],
            year_data["Municipal Group"],
        )
        plt.title(
            f"Scatter Plot: Normalized Total Households Serviced vs Normalized Operation Cost - Year {year}"
        )
        plt.tight_layout()
        # Remove the grid
        plt.grid(False)
...
def visualize_households_vs_operation_cost_per_tonnes(market_agg, unique_years):
    ...
    for year in unique_years:
        ...
        year_data["Total Households Serviced per operation cost"] = (
            year_data["Total Households Serviced"] / year_data["operation cost"]
        )

        # Normalize 'Normalized Total Households Serviced per operation cost' for the current year
        normalized_Total_Households_operation_cost = (
            year_data["Total Households Serviced per operation cost"]
            - year_data["Total Households Serviced per operation cost"].min()
        ) / (
            year_data["Total Households Serviced per operation cost"].max()
            - year_data["Total Households Serviced per operation cost"].min()
        )
        ...
        plt.title(
            f"Scatter Plot: Normalized Total Households Serviced per operation cost vs "
...
def visualize_interaction_with_operation_cost(market_agg, unique_years, threshold=3):
    ...
    for year in unique_years:
        ...
        # Calculate 'Total Households Serviced per operation cost' for the current year
        year_data.loc[:, "Total Households Serviced * operation cost"] = (
            year_data["Total Households Serviced"] * year_data["operation cost"]
        )
        ...
        if not year_data.empty:
            ...
            if not year_data.empty:
                ...
                plt.title(
                    f"Scatter Plot: Normalized Total Households Serviced * operation cost vs "
